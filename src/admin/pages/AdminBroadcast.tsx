...
const AdminBroadcast: React.FC = () => {
  ...
  const [audience, setAudience] =
    useState<Announcement["target_audience"]>("all");
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("announcements")
      .select(
        "id, title, message, type, is_active, target_audience, target_user_id, created_at, expires_at",
      )
  ...
  const handleRunReminders = async () => {
    ...
    if (!error) {
      ...
      setToast({
        kind: "error",
        text: "Failed to send reminders: " + error.message,
      });
    }
  ...
  return (
    <div className="relative space-y-6">
      ...
      {toast && (
        <div
          className={cn(
            "rounded-brand-lg fixed top-4 left-1/2 z-50 flex max-w-[90vw] -translate-x-1/2 items-center gap-2 border px-4 py-3 text-sm font-medium shadow-lg",
            toast.kind === "success"
      ...
      {/* Composer */}
      <div className="bg-bgCard border-borderMuted rounded-brand-lg space-y-3 border p-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-textDim mr-1 text-[10px] font-bold tracking-widest uppercase">
            Show to:
          </span>
          {AUDIENCES.map((a) => (
            <button
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-textDim mr-1 flex items-center gap-1 text-[10px] font-bold tracking-widest uppercase">
      ...
      {/* Renewal reminders */}
      <div className="bg-bgCard border-borderMuted rounded-brand-lg flex flex-wrap items-center justify-between gap-3 border p-5">
        <div>
          <p className="text-textMain flex items-center gap-1.5 text-sm font-semibold">
            <Clock className="text-warn h-4 w-4" /> Renewal Reminders
          </p>
          <p className="text-textDim mt-0.5 text-xs">
            Sends a renewal notice to every Pro user expiring within 7 days who
        ...
        <button
          ...
          className="bg-warn/10 border-warn/20 text-warn hover:bg-warn/20 rounded-brand flex items-center gap-2 border px-4 py-2 text-sm font-semibold disabled:opacity-50"
        >
          {runningReminders ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : (
      ...
      <div className="bg-bgCard border-borderMuted rounded-brand-lg overflow-hidden border">
        <div className="divide-borderMuted max-h-130 divide-y overflow-y-auto">
          {loading ? (
            <div className="flex justify-center py-10">
              <Loader2 className="text-brand h-5 w-5 animate-spin" />
            </div>
          ) : list.length === 0 ? (
            <p className="text-textDim py-10 text-center text-sm">
            ...
            list.map((a) => (
              <div key={a.id} className="flex items-start gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="text-textMain truncate text-sm font-semibold">
                    {a.title}
                  </p>
                  <p className="text-textDim mt-0.5 line-clamp-2 text-xs">
                    {a.message}
                  </p>
                  <p className="text-textDim mt-1 flex flex-wrap items-center gap-2 text-[10px]">
                    <span>
                    ...
                    <span className="border-borderMuted bg-bgSurface text-textDim rounded-full border px-2 py-0.5 text-[9px] font-bold tracking-wide uppercase">
                      {a.target_user_id
                        ? "Individual reminder"
                ...
                {a.is_active && (
                  <button
                    ...
                    className="text-textDim hover:text-danger shrink-0 p-1.5"
                    title="Deactivate"
                  >
        ...
        {!loading && hasMore && (
          <button
            ...
            className="text-textDim hover:text-textMain border-borderMuted flex w-full items-center justify-center gap-1.5 border-t py-3 text-xs font-semibold disabled:opacity-50"
          >
            {loadingMore ? (
