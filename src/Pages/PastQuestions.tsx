...
const OFFLINE_PACKS = [
  { id: 'eng-all',  subject: 'English',     years: '2010–2024', count: 240, size: '1.2 MB' },
  { id: 'math-all', subject: 'Mathematics', years: '2010–2024', count: 220, size: '1.0 MB' },
  { id: 'phy-all',  subject: 'Physics',     years: '2010–2024', count: 200, size: '0.9 MB' },
  { id: 'chem-all', subject: 'Chemistry',   years: '2010–2024', count: 210, size: '0.9 MB' },
  { id: 'bio-all',  subject: 'Biology',     years: '2010–2024', count: 190, size: '0.8 MB' },
];
...
const PastQuestions: React.FC = () => {
  ...
  const [filters, setFilters] = useState<Filters>({
    subject: 'All', year: 'All', topic: 'All', difficulty: 'All', search: '',
  });
  ...
  const filtered = useMemo(() => {
    return (SAMPLE_QUESTIONS as Question[]).filter((q) => {
      ...
      if (filters.search) {
        ...
        if (!q.text.toLowerCase().includes(q_) &&
            !q.topic.toLowerCase().includes(q_)) return false;
      }
      return true;
    });
  }, [filters]);
  ...
  const years      = ['All', ...new Set(SAMPLE_QUESTIONS.map((q) => String(q.year))).values()].sort((a, b) => b > a ? 1 : -1);
  const topics     = ['All', ...new Set(SAMPLE_QUESTIONS.map((q) => q.topic)).values()].sort();
  const subjects   = ['All', ...new Set(SAMPLE_QUESTIONS.map((q) => q.subject)).values()].sort();

  return (
    <AppLayout currentPage="past-questions" isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6">
        ...
        <div className="flex gap-1 bg-bgSurface border border-borderMuted rounded-brand p-1">
          ...
          >
            Offline packs
            {!isPro && <span className="text-[9px] px-1.5 py-0.5 bg-warn/15 text-warn rounded-full">PRO</span>}
          </button>
        </div>
      </div>
          ...
          {paginated.length === 0 ? (
            <div className="text-center py-16 text-textDim">
              ...
              <button
                ...
                onClick={() => handleFilterChange({ subject: 'All', year: 'All', topic: 'All', difficulty: 'All', search: '' })}
              >
                Clear all filters
              </button>
            </div>
          ...
          {!isPro ? (
            ...
            <>
              <div className="bg-success/10 border border-success/20 rounded-brand-lg px-4 py-3 flex items-center gap-2 mb-5 text-sm text-success">
                ✓ <span>Pro plan active — download any subject pack for offline access.</span>
              </div>
