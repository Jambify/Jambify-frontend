  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session) {
        ...
        navigate(s.onboardingComplete ? '/' : '/onboarding', { replace: true });
      }
    });
  }, []);
      ...
      >
              ...
              <div className="p-4 bg-danger/10 border border-danger/20 rounded-brand-lg flex items-start gap-3">
                ...
                <div>
                  ...
                  {error.includes('No account found') && (
                    <Link to="/signup" className="text-xs text-brand-light hover:underline mt-1 block">
                      Create an account →
                    </Link>
                  )}
        ...
        <div className="mt-8 text-center space-y-5">
          <p className="text-sm text-textDim">
            New to JAMBReady?{' '}
            <Link to="/signup" className="text-brand-light hover:underline font-semibold">Create Account</Link>
          </p>
