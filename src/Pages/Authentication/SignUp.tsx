...
import {
  Mail, ArrowRight, ShieldCheck, User,
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: statusData, error: statusErr } = await supabase
        .rpc('check_email_status', { p_email: email.trim().toLowerCase() });
      ...
      if (statusData?.exists === true && statusData?.confirmed === false) {
        const { data: cleanupData } = await supabase
          .rpc('cleanup_unverified_user', { p_email: email.trim().toLowerCase() });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.session) {
        ...
        navigate(state.onboardingComplete ? '/' : '/onboarding', { replace: true });
      }
    } catch (err) {
      ...
      <div className="min-h-screen bg-bgMain flex items-center justify-center p-4 relative overflow-hidden">
        ...
        >
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-display font-bold text-white mb-2 tracking-tight">Check your Email</h1>
            <p className="text-textDim text-sm">
              6-digit code sent to <span className="text-brand-light font-medium">{email}</span>
            </p>
          </div>

          <AnimatePresence mode="wait">
            {error && (
              <motion.div key="err" initial={{ opacity: 0, height: 0 }}
    ...
    <div className="min-h-screen bg-bgMain flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-textDim text-sm">No password needed — we email you a 6-digit code.</p>
        </div>
              ...
              <div className="p-4 bg-danger/10 border border-danger/20 rounded-brand-lg flex items-start gap-3">
                ...
                <div>
                  ...
                  {error.includes('already exists') && (
                    <Link to="/signin" className="text-xs text-brand-light hover:underline mt-1 block">
                      Sign in instead →
                    </Link>
                  )}
                </div>
        ...
        <form onSubmit={handleSendOtp} className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-textMuted mb-2 px-1">Full Name</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-textMuted mb-2 px-1">Email Address</label>
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
        ...
        <div className="mt-8 text-center space-y-5">
          <p className="text-sm text-textDim">
            ...
            <Link to="/signin" className="text-brand-light hover:underline font-semibold">Sign In</Link>
          </p>
