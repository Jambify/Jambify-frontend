...
const ReviewScreen: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <AppLayout currentPage="Review" isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen}>
      <div className="w-full max-w-full overflow-x-hidden box-border relative">
        <div className="max-w-5xl mx-auto px-4 py-6 md:px-8 overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-3 min-w-0">
              <Button variant="secondary" size="sm" onClick={onBack} className="rounded-xl shrink-0">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 max-w-full">
              {subjectData.map((sub) => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 px-2 py-2 rounded-xl text-xs font-bold border transition-all whitespace-nowrap shrink-0",
                    activeTab === sub.name ? "bg-brand border-brand text-white shadow-md" : "bg-bgCard border-borderMuted text-textDim hover:border-brand/40"
                  )}
                >
                  {sub.name} <span className="opacity-70 text-[10px]">{sub.score}/{sub.total}</span>
                </button>
              ))}
            </div>
              ...
              {["all", "correct", "incorrect"].map((f) => (
                <button
                  ...
                  className={cn(
                    ...
                    statusFilter === f ? "bg-textMain border-textMain text-bgCard" : "bg-transparent border-borderMuted text-textDim"
                  )}
                >
                  {f}
                </button>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isCorrect ? "border-l-4 border-l-success border-success/20" : "border-l-4 border-l-danger border-danger/20"
                  )}
                >
                  <div className="p-4 md:p-8">
                    <div className="flex items-start gap-3 md:gap-5 mb-6 min-w-0">
                      <div className={cn("w-8 h-8 md:w-12 md:h-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm mt-1", isCorrect ? "bg-success/10 text-success" : "bg-danger/10 text-danger")}>
                        {isCorrect ? <CheckCircle size={20} /> : <XCircle size={20} />}
                      </div>
                      <div className="min-w-0 flex-1">
                        <span className="text-[10px] font-mono font-bold text-textDim uppercase">Q {idx + 1}</span>
                        <p className="text-sm md:text-lg font-bold text-textMain mt-1 leading-snug wrap-wrap-break-words">{q.text}</p>
                      </div>
                    </div>
                      ...
                      {q.options.map((opt, i) => (
                        <div
                          ...
                          className={cn(
                            ...
                            i === q.answer ? "border-success bg-success/5 text-success font-bold" : i === userAnswer ? "border-danger bg-danger/5 text-danger" : "border-borderMuted text-textDim opacity-80"
                          )}
                        >
                          <span className="pr-2 wrap-wrap-break-words flex-1 min-w-0">
                            <span className="opacity-50 font-mono mr-1">{String.fromCharCode(65 + i)}.</span> {opt}
                          </span>
                    ...
                    <div className="md:ml-16 space-y-4">
                      <div className="p-4 bg-bgSurface/50 rounded-2xl border border-borderMuted overflow-hidden">
                        ...
                        <p className="text-xs md:text-sm text-textMuted leading-relaxed wrap-wrap-break-words">{q.explanation || "No explanation provided."}</p>
                      </div>
                      <div className="flex items-center justify-between gap-4">
                        <button onClick={() => handleAskAI(q)} className="flex items-center gap-2 px-4 py-2.5 bg-brand text-white rounded-xl shadow-lg shadow-brand/20 hover:scale-105 transition-all shrink-0">
                          <MessageCircle size={14} />
                          <span className="text-[11px] font-bold uppercase tracking-tight">Ask AI Helper</span>
                        </button>
                        <div className="text-right min-w-0">
                          <p className="text-[9px] text-textDim font-mono uppercase">Topic</p>
                          <p className="text-[10px] font-bold text-textMain truncate">{q.topic || "General"}</p>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        {selectedAIQuestion && (
          <div className="fixed inset-0 z-100 flex justify-end">
            <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={() => { setSelectedAIQuestion(null); setChatInput(""); setChatHistory([]); }} />
            <div className="relative w-full max-w-md bg-bgCard h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 z-10 overflow-hidden">
              <div className="p-6 border-b border-borderMuted flex items-center justify-between bg-brand text-white shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div><h2 className="font-bold">AI Tutor</h2><p className="text-[10px] opacity-80 uppercase tracking-widest font-mono">Interactive Help</p></div>
                </div>
                <button onClick={() => { setSelectedAIQuestion(null); setChatHistory([]); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><X size={24} /></button>
              </div>

              {/* Chat Body - Added ref={chatContainerRef} here */}
              <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth pb-10">
                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-brand uppercase">The Question</span>
                  <p className="text-sm font-medium text-textMain bg-bgSurface p-4 rounded-xl border border-borderMuted wrap-wrap-break-words">{selectedAIQuestion.text}</p>
                </div>
                <div className="space-y-3">
                  <span className="text-[10px] font-bold text-brand uppercase">AI Analysis</span>
                  {isAILoading ? (
                    ...
                    <div className="prose prose-sm text-textMuted leading-relaxed bg-brand/5 p-4 rounded-2xl border border-brand/10 wrap-break-words">
                      <p>Based on <strong>{selectedAIQuestion.topic || "General"}</strong>, option {String.fromCharCode(65 + selectedAIQuestion.answer)} is correct because:</p>
                      <p className="mt-2">{selectedAIQuestion.explanation || "No explanation provided."}</p>
                    </div>
                  )}
                </div>
                ...
                {chatHistory.length > 0 && (
                  <div className="space-y-4 pt-4 border-t border-borderMuted/30">
                    {chatHistory.map((msg, i) => (
                      <div key={i} className={cn("flex flex-col max-w-[85%] animate-in fade-in slide-in-from-bottom-2 duration-300", msg.role === "user" ? "ml-auto items-end" : "mr-auto items-start")}>
                        <span className="text-[9px] font-bold text-textDim uppercase mb-1 px-1">{msg.role === "user" ? "You" : "AI Tutor"}</span>
                        <div className={cn("p-3 rounded-2xl text-sm leading-relaxed wrap-break-words", msg.role === "user" ? "bg-brand text-white rounded-tr-none shadow-sm" : "bg-bgSurface border border-borderMuted text-textMain rounded-tl-none")}>
                          {msg.content}
                        </div>
                      </div>
                    ))}
                ...
                <div className="pt-4 text-center">
                  <p className="text-[10px] text-textDim italic">{chatHistory.length === 0 ? "Ask a follow-up question below." : "Keep the conversation going!"}</p>
                </div>
              </div>
              ...
              <div className="p-4 md:pb-4 pb-24 bg-bgCard border-t border-borderMuted shrink-0">
                <div className="relative flex items-center gap-2 bg-bgSurface border border-borderMuted rounded-2xl p-2 focus-within:border-brand transition-colors">
                  ...
                  <button
                    ...
                    className={cn("p-2 rounded-xl transition-all shrink-0", chatInput.trim() ? "bg-brand text-white shadow-md shadow-brand/20 hover:scale-105 active:scale-95" : "bg-borderMuted text-textDim cursor-not-allowed")}
                  >
                    <MessageCircle size={18} />
                  </button>
                </div>
                <p className="mt-2 text-[9px] text-center text-textDim uppercase tracking-tighter">AI can make mistakes. Verify important information.</p>
              </div>
            </div>
          </div>
        )}
