...
const mediaFrom = (apiQuestion: ApiQuestionShape) =>
  textFrom(
    apiQuestion.image,
...
export const mapApiQuestion = (
  ...
  const option = (apiQuestion.option ||
    apiQuestion.options ||
...
const parseALOCResponse = async (response: Response) => {
  ...
  const data = Array.isArray((payload as { data?: unknown })?.data)
    ? ((payload as { data: unknown[] }).data as ApiQuestionShape[])
    : (payload as { data?: ApiQuestionShape })?.data
      ? [(payload as { data: ApiQuestionShape }).data]
      : [];

  return {
    status:
      ((payload as { status?: unknown })?.status as number | undefined) ||
      response.status,
    data,
    message:
...
export const fetchFromALOC = async ({
  ...
  const fetchEndpoint = async (endpoint: "m" | "q") => {
    ...
    return result.data.map((item: ApiQuestionShape) =>
      mapApiQuestion(
        item,
        subject,
        safeYear === "Random" ? (item.examyear as string) || "" : safeYear,
      ),
    );
  };
...
export type { UniversityDomainShape };
