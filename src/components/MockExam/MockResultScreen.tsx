...
const MockResultsScreen: React.FC<MockResultsScreenProps> = ({ onRetry, onHome }) => {
  ...
  const getPerformanceBg = (performance: string) => {
    switch (performance) {
      ...
      case 'Good': return 'bg-brand/10 border-brand/20';
      case 'Average': return 'bg-warning/10 border-warning/20';
      case 'Poor': return 'bg-danger/10 border-danger/20';
      default: return 'bg-bgSurface border-borderMuted';
    }
  };
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6">
      ...
      <div className="relative overflow-hidden bg-bgCard border border-borderMuted rounded-brand-2xl p-10 mb-10 text-center shadow-2xl">
        ...
        <div className="relative z-10">
          <h2 className="text-sm font-black text-textDim uppercase tracking-[0.3em] mb-6">Unified Mock Result</h2>
          ...
          <div className="flex flex-wrap justify-center gap-8 mt-12 pt-10 border-t border-borderMuted/50">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-textDim uppercase tracking-widest mb-1">Accuracy</span>
              <span className="text-3xl font-display font-black text-textMain">{percentageScore}%</span>
            </div>
            <div className="w-px h-12 bg-borderMuted hidden sm:block"></div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-textDim uppercase tracking-widest mb-1">Correct</span>
              <span className="text-3xl font-display font-black text-textMain">{totalCorrect} / {totalQuestions}</span>
            </div>
            <div className="w-px h-12 bg-borderMuted hidden sm:block"></div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-textDim uppercase tracking-widest mb-1">Status</span>
              <span className={cn("text-3xl font-display font-black", getPerformanceColor(subjectBreakdown[0]?.performance || 'Good'))}>
                {jambScore >= 250 ? 'Excellent' : jambScore >= 200 ? 'Good' : 'Average'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
        {subjectBreakdown.map((sb) => (
          <div key={sb.subject} className="group bg-bgCard border border-borderMuted rounded-brand-xl p-6 flex flex-col gap-4 hover:border-brand/30 transition-all shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-black text-lg tracking-tight group-hover:text-brand transition-colors">{sb.subject}</h3>
                <div className={cn("inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border mt-1", getPerformanceBg(sb.performance), getPerformanceColor(sb.performance))}>
                  {sb.performance}
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-display font-black text-textMain">{sb.score}%</div>
                <div className="text-[10px] text-textDim uppercase font-black">{sb.correct} / {sb.total}</div>
              </div>
            </div>
      ...
      {/* Question Review - Professional Accordion */}
      <h3 className="text-xs font-black text-textDim uppercase tracking-widest mb-6 ml-1 flex items-center gap-2">
        <div className="w-1.5 h-1.5 rounded-full bg-brand"></div>
        Detailed Review
      </h3>
      <div className="space-y-4 mb-16">
        {subjectBreakdown.map((sb) => (
          <div key={`review-${sb.subject}`} className="border border-borderMuted rounded-brand-xl overflow-hidden bg-bgCard shadow-sm">
            <button 
            ...
            >
              <div className="flex items-center gap-4">
                <div className={cn("w-10 h-10 rounded-lg border flex items-center justify-center font-display font-black text-sm", getPerformanceBg(sb.performance), getPerformanceColor(sb.performance))}>
                  {sb.score}
                </div>
                <div className="text-left">
                  <span className="font-black text-sm block tracking-tight">{sb.subject} Review</span>
                  <span className="text-[10px] text-textDim uppercase font-bold tracking-widest">{sb.total} Questions • {sb.correct} Correct</span>
                </div>
              </div>
              <div className="w-8 h-8 rounded-full bg-bgSurface border border-borderMuted flex items-center justify-center group-hover:border-brand/30 transition-all">
                {expandedSubject === sb.subject ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </div>
            </button>
                    ...
                    return (
                      <div key={q.id} className="relative">
                        <div className="flex items-start gap-4 mb-4">
                          <div className={cn(
                            "flex-shrink-0 w-8 h-8 rounded-lg border flex items-center justify-center text-xs font-black shadow-sm",
                            isCorrect ? "bg-success/10 border-success/30 text-success" : 
                            isUnanswered ? "bg-bgSurface border-borderMuted text-textDim" : "bg-danger/10 border-danger/30 text-danger"
                          )}>
                            {idx + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm sm:text-base font-bold text-textMain leading-relaxed mb-6">{q.text}</p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {q.options.map((opt, optIdx) => (
                                ...
                                >
                                  ...
                                  {optIdx === q.answer && <CheckCircle size={16} className="shrink-0" />}
                                  {optIdx === userAnswer && optIdx !== q.answer && <XCircle size={16} className="shrink-0" />}
                                </div>
                              ))}
                            </div>
                              ...
                              <div className="flex items-center gap-2">
                                {isCorrect ? (
                                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-success flex items-center gap-2">
                                    <div className="w-1 h-1 rounded-full bg-success"></div> Correct Response
                                  </span>
                                ) : isUnanswered ? (
                                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-textDim flex items-center gap-2">
                                    <div className="w-1 h-1 rounded-full bg-textDim"></div> No Response Provided
                                  </span>
                                ) : (
                                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-danger flex items-center gap-2">
                                    <div className="w-1 h-1 rounded-full bg-danger"></div> Incorrect Response
                                  </span>
                                )}
                              </div>
                              {q.explanation && (
                                <div className="bg-brand/5 p-5 rounded-xl text-xs sm:text-sm text-textMain border border-brand/10 leading-relaxed shadow-inner">
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="font-black uppercase tracking-widest text-[10px] text-brand">Explanation</span>
                                  </div>
                        ...
                        {idx !== questions.filter(quest => quest.subject === sb.subject).length - 1 && (
                          <div className="h-px bg-gradient-to-r from-transparent via-borderMuted to-transparent my-10 opacity-30"></div>
                        )}
                      </div>
                    );
                  })}
      ...
      {!isAuthenticated && (
        <div className="relative overflow-hidden bg-brand/5 border border-brand/10 rounded-brand-2xl p-10 mb-12 text-center shadow-inner">
          ...
          <h4 className="text-xl font-black text-brand mb-3 tracking-tight">Save your official performance!</h4>
          <p className="text-sm text-textDim mb-8 max-w-md mx-auto leading-relaxed">Create a free account to track your JAMB score progress, unlock AI-powered explanations, and compete on the leaderboard.</p>
          <div className="flex justify-center">
            <Button variant="primary" size="lg" className="px-10 shadow-lg shadow-brand/20 font-black text-sm">
              Create Account
            </Button>
          </div>
        </div>
