    ...
    <div className="max-w-xl mx-auto animate-fadeIn">
      ...
      <div className="relative bg-bgCard border border-borderMuted rounded-brand-xl p-8 text-center mb-5 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(circle at 50% 0%, rgba(91,59,255,0.08) 0%, transparent 70%)' }} />
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div key={label} className="bg-bgCard border border-borderMuted rounded-brand-lg p-4 text-center">
            <div className={cn('font-display text-2xl font-bold tracking-tight', color)}>{value}</div>
      ...
      <div className="bg-bgCard border border-borderMuted rounded-brand-lg overflow-hidden mb-5">
        ...
        {questions.map((q, i) => {
          ...
          return (
            <div key={q.id} className="flex items-start gap-3 px-4 py-3 border-b border-borderMuted last:border-b-0">
              <span className={cn(
                'w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5',
                wasCorrect ? 'bg-success/15 text-success'
                  : skipped ? 'bg-white/5 text-textDim'
