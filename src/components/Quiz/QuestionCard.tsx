    ...
    <div className="animate-fadeIn">
      ...
      <div className="bg-bgCard border border-borderMuted rounded-brand-xl p-5 sm:p-7 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
          <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${subjStyle.bg} ${subjStyle.text}`}>
            {q.subject}
          </span>
          <span className="text-[11px] font-mono text-textDim">JAMB {q.year}</span>
          <span className={`text-[11px] ${DIFF_STYLES[q.difficulty] ?? 'text-textDim'}`}>
            ● {q.difficulty}
          </span>
      ...
      {/*   <{/* Hint button */} 
        {!hasAnswered && (
          <button
            className="flex items-center gap-1.5 text-xs text-brand-light border border-brand/25 hover:bg-brand/10 px-3 py-2 rounded-brand transition-all"
            onClick={() => {
              const firstSentence = q.explanation.split('.')[0] + '.';
              alert('Hint: ' + firstSentence);
            }}
          >
